...
updateimage = r'monstergulf_{}.png'.format(time.strftime('%Y_%m_%d_%H.%M.%S'))    
logger = logging.getLogger(__name__)

logger.info('MonsterGulf Start')
...
def loginUpload(username, password, path_to_resume):
    ...
    try:
        ...
        logger.error('Error in monstergulf cv upload- {}'.format(ex))
    finally:        
        browser.quit()
        logger.info('MonsterGulf End')    